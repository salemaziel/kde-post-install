...
class CreateComponent {
  ...
  createComponent() {
    fs.readFile(this.template, 'utf8', (error, content) => {
      ...
      this.createTestIfNeeded(this.outputInfo);

      this.openFileIfNeeded();
    });
  }

  createTestIfNeeded(componentOutputInfo) {
    if (config().has('tests') && config().get('tests')) {
      const testPath = determinePath(this.fileName, true);
      new CreateTest(testPath.fileName);
    }
  }
