...
const determinePath = (fileName, isTestPath = false) => {
  ...
  switch (true) {
    case ROOT_DIR.test(baseOutputPath):
      return path.join(baseOutputPath.replace(ROOT_DIR, PROJECT_ROOT), outputFileName);
    case COMPONENT_DIR.test(baseOutputPath):
      return path.join(baseOutputPath.replace(COMPONENT_DIR, CURRENT_FILE_DIR), outputFileName);
    default:
