(function (define) {
	define(['module', 'exports', './remap', './writeReport', './checkThreshold', '../utils/node!istanbul/lib/store/memory', '../utils/node!plugin-error', '../utils/node!through2'], function (module, exports, _remap, _writeReport, _checkThreshold, _memory, _nodePluginError, _nodeThrough) {
		'use strict';
		...
		var _nodePluginError2 = _interopRequireDefault(_nodePluginError);
			...
			return _nodeThrough2.default.obj(function (file, enc, cb) {
				if (!opts.warn) {
					opts.warn = function (message) {
						if (opts.fail) {
							cb(new _nodePluginError2.default('remap-istanbul', message));
						} else {
				...
				if (file.isStream()) {
					cb(new _nodePluginError2.default('remap-istanbul', 'Streaming not supported'));
				}
				...
				Promise.all(p).then(function () {
					if (thresholdCheckFailed) {
						return cb(new _nodePluginError2.default('remap-istanbul', 'Coverage threshold not met'));
					} else {
						cb(null, file);
					}
				});
